...
int onion_hermit_stack_init(struct onion_worker_head_t *head) {
   ...
   head->hermit_stack->initialized = true;
   ...
   if (!hermit_stack->sock_data) {
      DEBUG_ERR("Failed to initialize sock data.\n");
      goto free_stack;
   }
...
void onion_hermit_stack_exit(onion_hermit_stack_t *hermit_stack) {
   ONION_CHECK_AND_SET_STRUCT_INITIALIZE(hermit_stack); 
   hermit_stack->should_stop = true;

   if (hermit_stack->flow) {
      pthread_cancel(hermit_stack->flow);
...
int onion_worker_slot_init(struct onion_worker_head_t *head, int max_peers) {
   ...
   if (!epoll_static || !net_static || !workers) {
      DEBUG_ERR("Workers block or statics not allocated in worker stack.\n");
      goto fail;
   }
...
void onion_worker_slot_exit(struct onion_block *workers, struct onion_worker_t *worker) {
   ONION_CHECK_AND_SET_STRUCT_INITIALIZE(worker);
   ...
   ONION_CHECK_STRUCT_INITIALIZE(workers);
...
int onion_workers_init(struct onion_worker_head_t *head) {
   ...
   size_t worker_size = sizeof(struct onion_worker_t);
   size_t total_size  = worker_size * capable;

   struct onion_block *workers = onion_block_init(total_size, worker_size);
   if (!workers) {
...
void onion_workers_exit(struct onion_block *workers) {
   ONION_CHECK_AND_SET_STRUCT_INITIALIZE(workers);
   ...
   while ((index = onion_find_bit(workers->bitmask, 1)) != (size_t)-1) {
      ...
      if (!worker || !worker->initialized) {
         DEBUG_ERR("Null or uninitialized worker at index %zu\n", index);
...
int onion_worker_stack_init(struct onion_worker_head_t *head) {
   ...
   head->worker_stack->initialized = true;
...
void onion_worker_stack_exit(onion_worker_stack_t *worker_stack) {
   ONION_CHECK_AND_SET_STRUCT_INITIALIZE(worker_stack);
...
void onion_device_exit(struct onion_worker_head_t *head) {
   ONION_CHECK_AND_SET_STRUCT_INITIALIZE(head);
